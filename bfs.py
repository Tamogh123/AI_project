...
while True:
    ...
    # draw grid
    [[pg.draw.rect(sc, pg.Color('grey'), get_rect(x, y), border_radius=TILE // 5)
      for x, col in enumerate(row) if col] for y, row in enumerate(grid)]
    # draw BFS work
    [pg.draw.rect(sc, pg.Color('white'), get_rect(x, y)) for x, y in visited]
    ...
    while path_segment and path_segment in visited:
        pg.draw.rect(sc, pg.Color('black'), get_rect(*path_segment), TILE, border_radius=TILE // 3)
